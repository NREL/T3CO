...
T3CO contains three main input files and several auxiliary files that are referenced in the main files for different purposes. T3CO provides users with demo input and auxiliary files to get started. These files can be used as templates for the user to define their own scenarios and analyses by editing individual inputs, adding rows, or creating entirely new files using the corresponding template structure. When creating new files, the user needs to specify the filepath(s) in the [Config input file](./config_inputs_descriptions.md).

### Vehicle, Scenario, and Config

The main input files are the [***Vehicle***](vehicle_inputs_descriptions.md), [***Scenario***](scenario_inputs_descriptions.md), and [***Config***](config_inputs_descriptions.md). One or more *Vehicle-Scenario* pair selections are necessary to run T3CO:
