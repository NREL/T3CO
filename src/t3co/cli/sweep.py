...
if __name__ == "__main__":
    ...
    if args.config is None or args.config == "None":
        ...
        config.vehicle_file = (Path(config.vehicle_file) if Path(config.vehicle_file).is_absolute() else gl.RESOURCES_FOLDERPATH / config.vehicle_file)
        config.scenario_file = (Path(config.scenario_file) if Path(config.scenario_file).is_absolute() else gl.RESOURCES_FOLDERPATH / config.scenario_file)
        config.eng_eff_imp_curves = (Path(config.eng_eff_imp_curves) if Path(config.eng_eff_imp_curves).is_absolute() else gl.RESOURCES_FOLDERPATH / config.eng_eff_imp_curves)
        config.lw_imp_curves = (Path(config.lw_imp_curves) if Path(config.lw_imp_curves).is_absolute() else gl.RESOURCES_FOLDERPATH / config.lw_imp_curves)
        config.aero_drag_imp_curves = (Path(config.aero_drag_imp_curves) if Path(config.aero_drag_imp_curves).is_absolute() else gl.RESOURCES_FOLDERPATH / config.aero_drag_imp_curves)
