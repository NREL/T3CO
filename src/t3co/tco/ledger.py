...
class Ledger():
    ...
    config: Config = None
    energy: Energy = None
    def __init__(self, vehicle: Vehicle, scenario: Scenario, energy: Energy = None, config: Config = None):
        ...
        for year_index in range(self.vehicle_life_yr):
            self.tco_per_year.append(TCOCalc(year_index=year_index, vehicle=self.vehicle, scenario=self.scenario, energy=self.energy, cap_costs=(self.tco_per_year[year_index-1].cap_costs_dol if year_index else None)))
