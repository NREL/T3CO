...
from t3co.cli.sweep import load_vehicle_scenario_energy, generate_ledger, create_results_filepath, export_results_to_csv, run_t3co
...
def test_generate_ledger(mocker, config, vehicle, scenario, energy):
    mocker.patch('t3co.cli.sweep.load_vehicle_scenario_energy', return_value=(vehicle, scenario, energy))
...
def test_run_t3co(mocker, config):
    mocker.patch('t3co.cli.sweep.generate_ledger', return_value={"selection": 1, "value": 100})
    mocker.patch('t3co.cli.sweep.export_results_to_csv', return_value=(Path("results.csv"), pd.DataFrame([{"selection": 1, "value": 100}])))
