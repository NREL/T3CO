...
def test_main_copy_files(mocker):
    ...
    mock_copy_demo_input_files = mocker.patch("t3co.utils.demo_files_installer.copy_demo_input_files")
...
def test_copy_demo_input_files(mocker):
    ...
    mock_copy.assert_called_once_with(mock_file, Path("/path/to/destination/demo_inputs/file.txt"))
    mock_copytree.assert_called_once_with(mock_dir, Path("/path/to/destination/demo_inputs/dir"))
