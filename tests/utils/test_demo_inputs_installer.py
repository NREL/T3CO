...
def test_main_copy_files():
    with (
        patch("builtins.input", side_effect=["y", "/path/to/destination"]),
        patch(
            "t3co.utils.demo_inputs_installer.copy_demo_input_files"
        ) as mock_copy_demo_input_files,
    ):
        main()
        mock_copy_demo_input_files.assert_called_once_with("/path/to/destination")


def test_main_no_copy_files(capsys):
    with (
...
def test_copy_demo_input_files():
    with (
...
def test_copy_demo_input_files_existing_destination():
    with (
        patch("os.makedirs") as mock_makedirs,
        patch("shutil.copy") as mock_copy,
        patch("shutil.copytree") as mock_copytree,
